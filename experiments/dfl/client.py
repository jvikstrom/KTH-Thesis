...
class Client:
    ...
    def train(self, batches: int = 1, batch_size: int = 32):
        for i in range(batches):
            batch = np.random.choice(self.indexes, batch_size) # len(self.train_data[0]) // 10
            #print(f"batch: {batch}")
            x,y = self.train_data
            x_batch = x[batch]
            y_batch = y[batch]
...
#        self.model.fit(*self.train_data, batch_size=len(self.train_data[0]) // 10, epochs=epochs, verbose=0)
