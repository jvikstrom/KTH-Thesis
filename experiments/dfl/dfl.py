...
def run_emnist(path: str, N, strategy, batches=1, iterations=100):
    # Load simulation data.
    train, test = tff.simulation.datasets.emnist.load_data()  # TODO: Set only_digits=False
    ...
    np.save(path, np.array(hyper.test_evals))


def run_emnist_fls(N, batches=1, iterations=100):
    # Load simulation data.
    train, test = tff.simulation.datasets.emnist.load_data()  # TODO: Set only_digits=False
    ...
    np.save("fls.data", np.array(metrics))
...
if __name__ == "__main__":
    ...
    iterations = 101
    batches = 10
    run_emnist("exchange-gossip-eval.data", N, ExchangeGossip, batches=batches, iterations=iterations)
    run_emnist("gossip-eval.data", N, Gossip, batches=batches, iterations=iterations)
    run_emnist_fls(N, batches=batches, iterations=iterations)
